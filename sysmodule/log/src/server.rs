...
fn ring() -> &'static mut LogRing {
    // SAFETY: log server is single-threaded (IPC server loop).
    unsafe { &mut *core::ptr::addr_of_mut!(RING) }
}

// --- Interrupt tracking for USART output ---
...
static mut TRACKER: InterruptTracker = InterruptTracker::new();

fn tracker() -> &'static mut InterruptTracker {
    // SAFETY: log server is single-threaded (IPC server loop).
    unsafe { &mut *core::ptr::addr_of_mut!(TRACKER) }
}
...
impl Log for LogResource {
    fn log(meta: Meta, level: LogLevel, data: idyll_runtime::Leased<idyll_runtime::Read, u8>) {
        ...
        ring.push(id, level, task_index as u16, &buf[..len], 0, time);
        notify_logs(level);
    }
    ...
    fn write(&mut self, _meta: Meta, data: idyll_runtime::Leased<idyll_runtime::Read, u8>) {
        ...
        let _ = data.read_range(0, &mut buf[..len]);
        let time = get_packed_time();
        ring().push(self.id, self.level, self.task_index as u16, &buf[..len], self.idx, time);
        self.idx += 1;
    }
    ...
    fn consume_since(
        ...
        for chunk in ring().iter_since(since_id) {
