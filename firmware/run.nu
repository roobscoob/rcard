...
def main [
    ...
    let $fw = if $with != null {
        ...
        # Clear stale log metadata before build (regenerated by proc macros).
        let log_meta = ($project | path join .work log_meta)
        if ($log_meta | path exists) { rm -rf $log_meta }
    ...
    let $ports = powershell -Command "[System.IO.Ports.SerialPort]::GetPortNames()" | lines

    if ($ports | is-empty) {
        error make { msg: "No serial ports found. Is the board connected?" }
    }

    let port = ($ports | get 0)

    sftool --port $port -c SF32LB52 write_flash ./build/sdmmc.img@0x10000000
}
