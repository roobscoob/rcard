...
pub fn resolve() -> Result<Vec<u16>, String> {
    ...
    let task_index_of =
        |name: &str| -> Option<u16> { task_names.iter().position(|t| t == name).map(|i| i as u16) };
    ...
    for (client_name, deps) in uses_obj {
        let deps = deps
            .as_array()
            .ok_or("expected array per client in uses JSON")?;
        let lists_us = deps.iter().any(|d| d.as_str() == Some(&server_name));
        if lists_us {
    ...
    for (other_server, their_peers) in peers_obj {
        ...
        let their_peers = their_peers
            .as_array()
            .ok_or("expected array per server in peers JSON")?;
        let lists_us = their_peers.iter().any(|p| p.as_str() == Some(&server_name));
        if lists_us {
