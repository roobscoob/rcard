...
/// Parse a peripheral name like "usart1", "i2c2", "display_en" into (kind, instance).
fn parse_peripheral_name(
    name: &str,
    signals: &serde_json::Map<String, serde_json::Value>,
) -> (String, Option<String>) {
    // Check if it has a "direction" field — that means it's a GPIO
    if signals.contains_key("direction") {
        let dir = signals["direction"].as_str().unwrap_or("out");
    ...
    let i = name.len()
        - name
            ...
            .count();
    if i < name.len() {
        (name[..i].to_string(), Some(name[i..].to_string()))
    } else {
        (name.to_string(), None)
    }
}
